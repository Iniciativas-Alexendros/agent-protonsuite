...
- **npm:** Activo (`npmPublish: true`) con Trusted Publishing OIDC (`Soluciones-Alexendros/protonsuite-tools` / `release.yml`). Bootstrap/rescate: `gh workflow run release.yml -f bootstrap-npm=true`. **No** `NPM_TOKEN` en CI. Ver [docs/publishing.md](./docs/publishing.md).
