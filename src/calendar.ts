...
export class CalendarClient {
  constructor(private opts: { serverUrl: string; username: string; password: string }) {}

  private createClient() {
    return new DAVClient({
      serverUrl: this.opts.serverUrl,
      credentials: { username: this.opts.username, password: this.opts.password },
    });
  }
  ...
  async listEvents(calendarUrl: string, from: Date, to: Date): Promise<CalEvent[]> {
    const client = this.createClient();
    await client.login();
  ...
  async createEvent(calendarUrl: string, event: { summary: string; start: Date; end: Date; location?: string; description?: string }): Promise<CalEvent> {
    const client = this.createClient();
    await client.login();
    const ics = [
    ...
    return { uid: "", summary: event.summary, start: event.start, end: event.end, location: event.location, description: event.description };
  }
}
