...
async function main(): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(
      `[FATAL] Invalid configuration: ${(err as Error).message}\n`,
    )
    process.stderr.write(
  ...
  if (!cfg.transport.authToken) {
    process.stderr.write(
      '[FATAL] HTTP mode requires MCP_AUTH_TOKEN. Generate one: openssl rand -hex 32\n',
    )
    process.exit(2)
  }
  ...
  if (
    ...
    process.stderr.write(
      '[FATAL] HTTP mode in production requires MCP_ALLOWED_ORIGINS to be set\n',
    )
    process.exit(2)
  }
  ...
  app.listen(cfg.transport.httpPort, cfg.transport.httpHost, () => {
    log.info(
      `MCP listening on http://${cfg.transport.httpHost}:${cfg.transport.httpPort}/mcp`,
    )
  })
}
