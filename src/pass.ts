...
async function execPass(args: string[], opts: { env: NodeJS.ProcessEnv; input?: string }): Promise<string> {
...
export class PassClient {
  ...
  constructor(opts: PassClientOptions, private readonly log: PassLogger) {
  ...
  async list(filter?: string): Promise<string[]> {
    ...
    const stdout = await execPass(args, { env: { ...process.env, PASSWORD_STORE_DIR: this.storeDir } });
  ...
  async get(path: string): Promise<string> {
    ...
    try {
      ...
      if (msg.includes("not in the password store") || msg.includes("not found")) {
   ...
   */
  async generate(path: string, length = 24): Promise<{ path: string; length: number }> {
    this.validatePath(path);
  ...
  async audit(): Promise<PassAuditResult> {
    ...
    if (!healthResult.ok) {
      return {
        ...
        duplicates: [],
        staleEntries: [],
        recommendations: [`Store no accesible: ${healthResult.error ?? "error desconocido"}`],
      };
    }
    ...
    if (weakPasswords.length > 0) {
      recommendations.push(`${weakPasswords.length} contraseñas débiles detectadas (<12 caracteres o poca variedad). Regenerar con 'proton_pass_generate'.`);
    }
    if (duplicates.length > 0) {
      recommendations.push(`${duplicates.length} contraseñas duplicadas detectadas. Revisar y unificar entradas.`);
    }
